...
URL に以下のクエリパラメータを付けると、内部メトリクスのサンプリングや負荷テストが有効化されます。

| URL パラメータ | 効果 |
|---|---|
| `?autoload=path/to/file.ply` | 起動時に自動でファイル読み込み |
...
| `&qual=N` | qualScale を N に固定(`0.75` / `1.0` / `1.5`) |
| `&stress=1\|2` | カメラ自動回転(再現可能な負荷テスト) |
| `&smooth=0` / `&smooth=1` | タッチ視点の CSS reprojection を強制 OFF / ON |
